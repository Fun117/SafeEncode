...
const sidebar = [
    ...
    { name: _locales('API Reference'), href: links_config['/docs/api-reference'], now: false, subItems: [
		...
		{
			name: _locales('decrypt'), href: links_config['/docs/api-reference/decrypt'], now: true
		}
	]},
...
export default function Page() {
    ...
    // headカスタム
	const Head_config = {
		title:`${_locales('decrypt')} | SafeEncode`,
	};
					...
					<Main>
						<>
                            <DocsPageGroup>
                                ...
                                <DocsContentGroup>
									<DocsContent>
											<h1 className='font-bold text-5xl'>{_locales('Decryption Endpoint')}</h1>
											<p>{_locales(`This endpoint is used to decrypt content using a provided key.`)}</p>
											<h1 className='font-bold text-3xl'>{_locales('Endpoint')}</h1>
											<UI_Code lang='bash' copy code={`GET /api/decryption`}/>
											<h1 className='font-bold text-3xl'>{_locales('Parameters')}</h1>
											<p><Code>key</Code>{_locales('(required): The decryption key.')}</p>
											<p><Code>content</Code>{_locales('(required): The content to be decrypted.')}</p>
											<h1 className='font-bold text-3xl'>{_locales('Response')}</h1>
											<p><Code>content</Code>{_locales(': The decrypted content.')}</p>
...
`fetch('https://${links_config.SiteOrigin}/api/decryption?key=<_key>&content=<_content>')
	...
	.catch(error => {
		console.error('${_locales('Error fetching data:')}', error);
                                    ...
                                    <DocsContentNavGroup>
                                        <DocsContentNavButton type='back' href={links_config['/docs/api-reference/encrypt']} Label={_locales('encrypt')}/>
                                    </DocsContentNavGroup>
                                </DocsContentGroup>
                            </DocsPageGroup>
						</>
